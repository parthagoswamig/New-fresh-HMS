...
export default function NewLabTestPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await laboratoryService.create(payload, tenant?.id || '');
  ...
  const handleChange = (
    ...
    // Auto-fill test name when lab test is selected
    if (name === 'labTestId') {
      const selectedTest = labTests.find(test => test.id === value);
      if (selectedTest) {
        setFormData((prev) => ({
          ...prev,
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
      <div className="mb-6">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">New Lab Test</h1>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <Card className="mb-6">
          <CardHeader>
            <CardTitle>Test Results (Optional)</CardTitle>
          </CardHeader>
          <CardContent className="space-y-4">
